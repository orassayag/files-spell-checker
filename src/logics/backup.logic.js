...
class BackupLogic {
  ...
  initiate() {
    ...
    this.backupTitle = textUtils.removeAllCharacters(
      textUtils.toLowerCase(process.argv[2]),
      '.'
    );
  ...
  async setParameters() {
    ...
    for (
      let i = 0;
      i < this.backupDataModel.backupMaximumDirectoryVersionsCount;
      i++
    ) {
  ...
  async runBackup() {
    ...
    await fileUtils.removeDirectoryIfExists(
      this.backupDataModel.targetFullPath
    );
    await fileUtils.createDirectoryIfNotExists(
      this.backupDataModel.targetFullPath
    );
    // Create the standard backup.
    await fileUtils.copyDirectory(
      this.backupDataModel.sourceFullPath,
      this.backupDataModel.targetFullPath,
      this.filterDirectories.bind(this)
    );
  ...
  filterDirectories(source) {
    let isIncluded = true;
    const { ignoreDirectories, ignoreFiles, includeFiles } =
      this.backupDataModel.backupDirectoryModel;
  ...
  async verifyBackup() {
    await globalUtils.sleep(
      this.backupDataModel.millisecondsDelayVerifyBackupCount
    );
