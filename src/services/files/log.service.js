...
class LogService {
  ...
  updateFileName() {
    ...
    this.itemResultsPath = this.createFilePath(
      `${this.fileIndex}_${textUtils.toLowerCase(applicationService.applicationDataModel.method)}_${PlaceholderEnum.DATE}`
    );
  }
  ...
  getNextDirectoryIndex() {
    ...
    this.directoryIndex =
      Math.max(...directories.map((name) => textUtils.getSplitNumber(name))) +
      1;
  }
    ...
    return textUtils.cutText({
      ...
      count:
        countLimitService.countLimitDataModel
          .maximumItemNamePathCharactersDisplayCount,
    });
  }
  ...
  logProgress() {
    ...
    const currentPercentage = textUtils.calculatePercentageDisplay({
      partialValue: applicationService.applicationDataModel.itemIndex,
      totalValue: itemService.itemDataModel.totalItemsCount,
    });
    const current = textUtils.getNumberOfNumber({
      number1: applicationService.applicationDataModel.itemIndex,
      number2: itemService.itemDataModel.totalItemsCount,
    });
    ...
    const itemName = this.getDisplayItem(
      applicationService.applicationDataModel.itemName
    );
    const itemDirectoryPath = this.getDisplayItem(
      applicationService.applicationDataModel.itemDirectoryPath
    );
  ...
  updateSessionDirectoryPath() {
    ...
    if (
      this.logCounts %
        countLimitService.countLimitDataModel.maximumLogsCountPerFile ===
      0
    ) {
      this.updateFileName();
    }
