...
class ScanService {
  ...
  async run() {
    const isName =
      applicationService.applicationDataModel.method === MethodEnum.NAME;
    ...
    items = items.slice(
      0,
      countLimitService.countLimitDataModel.maximumItemsCount
    );
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      applicationService.applicationDataModel.itemCheckResult =
        checkResultsModel.resultsList[0];
    }
  ...
  scanItemName(itemPath, index) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        const { itemName, itemFullName, itemDirectoryPath } =
          fileUtils.getItemName(itemPath);
        ...
        for (let i = 0; i < wordsList.length; i++) {
          ...
          checkResultsModel.isLimitExceeded =
            itemService.itemDataModel.scanWordsCount >
            countLimitService.countLimitDataModel.maximumWordsScanCount;
          if (checkResultsModel.isLimitExceeded) {
            break;
          }
        }
        await globalUtils.sleep(
          countLimitService.countLimitDataModel
            .millisecondsBetweenItemsDelayCount
        );
  ...
  scanItemContent(itemPath, index) {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        if (
          this.allowFileExtensions.findIndex(
            (file) => file === fileType.toLowerCase()
          ) === -1
        ) {
        ...
        itemService.itemDataModel.scanItemsCount++;
        const checkResultsModel = new CheckResultsModel(itemPath);
        const { itemFullName, itemDirectoryPath } =
          fileUtils.getItemName(itemPath);
        applicationService.applicationDataModel.itemName = itemFullName;
        applicationService.applicationDataModel.itemDirectoryPath =
          itemDirectoryPath;
